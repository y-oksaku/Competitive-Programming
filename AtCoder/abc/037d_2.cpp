...
int main(void) {
    ...
    llongVV A(H + 2, llongV(W + 2, -LINF));
    vector<tuple<llong, llong, llong>> order;
    FORS(h, 1, H + 1) {
        FORS(w, 1, W + 1) {
            cin >> A[h][w];
            order.emplace_back(A[h][w], h, w);
        }
    }

    llongVV B(H + 2, llongV(W + 2, 1LL));
    ...
    for(auto e : order) {
        ...
        if(A[h + 1][w] > a) B[h + 1][w] = (B[h + 1][w] + n) % MOD;
        if(A[h - 1][w] > a) B[h - 1][w] = (B[h - 1][w] + n) % MOD;
        if(A[h][w + 1] > a) B[h][w + 1] = (B[h][w + 1] + n) % MOD;
        if(A[h][w - 1] > a) B[h][w - 1] = (B[h][w - 1] + n) % MOD;
    }
