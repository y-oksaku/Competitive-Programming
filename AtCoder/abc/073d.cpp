...
llong search(llong now, vector<bool> confilm) {
    confilm[now] = true;
    llong ret = INF;
    FOR(i, confilm.size()) {
        if(!confilm[i]) {
            ret = min(ret, search(i, confilm) + minDist[visit[now]][visit[i]]);
        }
    }
    if(ret == INF) {
        ret = 0;
...
int main(void) {
    ...
    llong ans = INF;
    FOR(i, R) {
        vector<bool> confilm(R, false);
        ans = min(ans, search(i, confilm));
    }
