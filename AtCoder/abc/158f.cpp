...
llong dfs(llong i) {
    if(V.find(i) == V.end()) return 0;
    V.erase(i);

    llong ret = 1;
    for(auto to = V.lower_bound(i + 1); to != V.end();){
        llong v = *to;
        if(v >= R[i]) break;
        ret = (ret * (dfs(v) + 1)) % MOD;
        to = V.lower_bound(v + 1);
    }
    return ret;
}
...
int main(void) {
    ...
    FOR(i, N) {
        llong x, d;
        tie(x, d) = XD[i];
        R[i] = lower_bound(X.begin(), X.end(), x + d) - X.begin();
    }
