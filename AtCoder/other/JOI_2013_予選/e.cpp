...
int main(void) {
    ...
    FOR(i, M) {
        ...
        cin >> fr >> to;
        fr--; to--;
        roads[fr].emplace_back(to);
        roads[to].emplace_back(fr);
    }

    vector<vector<int>> edges(N, vector<int>(0));
    FOR(fr, N) {
        deque<pair<int, int>> que;
        que.emplace_back(fr, 0);
        vector<bool> visited(N, false);

        while(!que.empty()) {
            int now, dist;
            tie(now, dist) = que.front();
            que.pop_front();

            if(dist == CR[fr].second) continue;

            for(int to : roads[now]) {
                if(visited[to]) continue;
                visited[to] = true;
                que.emplace_back(to, dist + 1);
            }
        }
        FOR(to, N) {
            if(visited[to]) edges[fr].emplace_back(to);
        }
    }

    vector<int> minDist(N, IINF);
    vector<bool> visited(N, false);
    minDist[0] = 0;
    ...
    FOR(cnt, N) {
        ...
        int cost = CR[now].first;
        for(int to : edges[now]) {
            chmin(minDist[to], mi + cost);
        }
    }
